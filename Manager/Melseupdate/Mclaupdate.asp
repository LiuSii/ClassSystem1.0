...
If (CStr(Request("MM_update")) = "get") Then
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "UPDATE dbo.Classroom SET Clno = ?, Clcapa = ?, Clsite = ? WHERE Clno = ?" 
...
If (CStr(Request("MM_delete")) = "del" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    Set MM_editCmd = Server.CreateObject ("ADODB.Command")
    MM_editCmd.ActiveConnection = MM_room_STRING
    MM_editCmd.CommandText = "DELETE FROM dbo.Classroom WHERE Clno = ?"
...
%>
<%
Dim Mupdatecla__MMColParam
Mupdatecla__MMColParam = "1"
If (Request.Form("no") <> "") Then 
  Mupdatecla__MMColParam = Request.Form("no")
End If
%>
<%
Dim Mupdatecla
Dim Mupdatecla_cmd
Dim Mupdatecla_numRows

Set Mupdatecla_cmd = Server.CreateObject ("ADODB.Command")
Mupdatecla_cmd.ActiveConnection = MM_room_STRING
Mupdatecla_cmd.CommandText = "SELECT * FROM dbo.Classroom WHERE Clno = ?" 
Mupdatecla_cmd.Prepared = true
Mupdatecla_cmd.Parameters.Append Mupdatecla_cmd.CreateParameter("param1", 200, 1, 50, Mupdatecla__MMColParam) ' adVarChar

Set Mupdatecla = Mupdatecla_cmd.Execute
Mupdatecla_numRows = 0
%>
		  ...
		  <form ACTION="<%=MM_editAction%>" METHOD="POST" name="get" id="get" >
 			<tr>
    			<td style="color:white">编号</td>
   			  <td><input name="id" type="text" id="id" value="<%=(Mupdatecla.Fields.Item("Clno").Value)%>" size="30" /></td>
 			</tr>
            <tr>
    			<td style="color:white">容量</td>
    			<td><input name="capacity" type="text" id="capacity" value="<%=(Mupdatecla.Fields.Item("Clcapa").Value)%>" size="30" /></td>
 			</tr>
            <tr>
    			<td style="color:white" onfocus="MM_validateForm('id','','R','capacity','','R','site','','R');return document.MM_returnValue">位置</td>
   			  <td><input name="site" type="text" id="site" value="<%=(Mupdatecla.Fields.Item("Clsite").Value)%>" size="30" /></td>  
		  	</tr>
  			<tr>
            ...
            <input type="hidden" name="MM_recordId" value="<%= Mupdatecla.Fields.Item("Clno").Value %>" />
          </form>
		</table>
