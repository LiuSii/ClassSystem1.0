...
If (CStr(Request("MM_delete")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  If (Not MM_abortEdit) Then
    ...
    MM_editCmd.CommandText = "DELETE FROM dbo.Log WHERE Lid = ?"
...
Recordset1_cmd.CommandText = "SELECT * FROM dbo.Log WHERE Lroom = ? and Ltime = 0" 
...
请再次确认教室编号<input name="name" type="text" id="name" value="<%=(Recordset1.Fields.Item("Lroom").Value)%>" />
<input name="id" type="hidden" value="<%=(Recordset1.Fields.Item("Lid").Value)%>" />
<input name="" type="submit" value="确认" />
<input type="hidden" name="MM_delete" value="form1" />
<input type="hidden" name="MM_recordId" value="<%= Recordset1.Fields.Item("Lid").Value %>" />
</form>
<div>
</body>
</html>
