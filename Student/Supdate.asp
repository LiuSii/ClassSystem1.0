...
%>
<%
Dim stuupdate__MMColParam
stuupdate__MMColParam = "1"
If (Session("MM_Username") <> "") Then 
  stuupdate__MMColParam = Session("MM_Username")
End If
%>
<%
Dim stuupdate
Dim stuupdate_cmd
Dim stuupdate_numRows

Set stuupdate_cmd = Server.CreateObject ("ADODB.Command")
stuupdate_cmd.ActiveConnection = MM_room_STRING
stuupdate_cmd.CommandText = "SELECT * FROM dbo.Student WHERE Sno = ?" 
stuupdate_cmd.Prepared = true
stuupdate_cmd.Parameters.Append stuupdate_cmd.CreateParameter("param1", 200, 1, 50, stuupdate__MMColParam) ' adVarChar

Set stuupdate = stuupdate_cmd.Execute
stuupdate_numRows = 0
%>
...
<body>
    ...
    <br>
   		...
   		<table width="400" border="0" align="center">
  			<form ACTION="<%=MM_editAction%>" METHOD="POST" name="get" id="get" onfocus="MM_validateForm('username','','R','name','','R','password','','R','iphone','','R');return document.MM_returnValue">
 			<tr>
    			<td style="color:white">用户名</td>
   			  <td><input name="username" type="text" id="username" value="<%=(stuupdate.Fields.Item("Sno").Value)%>" size="30" /></td>
 			</tr>
            <tr>
    			<td style="color:white">姓名</td>
    			<td><input name="name" type="text" id="name" value="<%=(stuupdate.Fields.Item("Sname").Value)%>" size="30" /></td>
 			</tr>
            <tr>
    			<td style="color:white">密码</td>
    			<td><input name="password" type="text" id="password" value="<%=(stuupdate.Fields.Item("Spassword").Value)%>" size="30" /></td>  
		  	</tr>
            <tr>
    			<td style="color:white">手机</td>
    			<td><input name="iphone" type="text" id="iphone" value="<%=(stuupdate.Fields.Item("Siphone").Value)%>" size="30" /></td>  
		  	</tr>
  			<tr>
            ...
            <input type="hidden" name="MM_recordId" value="<%= stuupdate.Fields.Item("Sno").Value %>" />
            </form>
		</table>
</body>
</html>
